# 进阶模块三：Query Rewrite 问题改写
# 架构：分层RAG + 多路召回 + 重排序 + 问题改写
import os
import json
import datetime
import jieba
...
# ===================== 2. 核心新增：Query Rewrite 问题改写 =====================
def rewrite_query(raw_query: str, history_msgs: list) -> str:
    """
    结合历史对话，改写原始问句：补全指代、口语转书面、标准化检索语句
    """
    # 截取最近N轮对话，避免上下文过长
...
任务：
1. 结合上文，补全指代、省略的内容
2. 将口语化语句改写为通顺、标准的书面检索问句
3. 不要回答问题，只输出改写后的**单个问句**
4. 禁止额外解释、符号、多余文字
"""
...
def load_memory():
    if os.path.exists(MEMORY_FILE):
        with open(MEMORY_FILE, "r", encoding="utf-8") as f:
            return json.load(f)
    return [{"role": "system", "content": "你是全能AI助手，依据参考资料如实回答。"}]
...
<head>
    <meta charset="UTF-8">
    <title>Query Rewrite + 层级RAG</title>
    <style>
...
<body>
    <h2>🔥 层级RAG + Query Rewrite 问题改写</h2>
...
def chat():
    user_raw_msg = request.json.get("msg")
    tool = agent_think(user_raw_msg)
    context = ""

    if tool == "search":
        # 关键链路：先改写问句 → 再检索
        rewrite_q = rewrite_query(user_raw_msg, messages)
        candidates = hierarchical_retrieval(rewrite_q)
        context = rerank_candidates(rewrite_q, candidates)
    elif tool == "calc":
        context = calculate(user_raw_msg)
    elif tool == "time":
        context = get_time()

    prompt = f"参考资料：{context}\n用户问题：{user_raw_msg}，请简洁回答。"
